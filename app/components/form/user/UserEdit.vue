    ...
    >
      ...
      <form @submit.prevent="onSubmit" class="p-6 space-y-6">
        <div class="grid grid-cols-1 gap-6">
          ...
          <!-- Password (optional) -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Kata Sandi (kosongkan jika tidak berubah)</label>
            <div class="relative">
              <input
...
const local = reactive<{name:string;email:string;password:string;role:string;isActive:boolean}>({
  name: '',
  email: '',
  password: '',
  role: '',
...
// watch for props.user or modal open to populate
watch(
  () => props.user,
  (u) => {
    if (u) {
