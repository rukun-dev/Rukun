...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    return responses.created({ user }, "Registration successful", {
      requestId,
      event,
      nextStep: {
        action: "login",
    ...
    if (error instanceof z.ZodError) {
      ...
      return responses.validation(
        ...
        {
          field_errors: zodError.issues.reduce(
            (acc, issue) => {
    ...
    if (error instanceof Error && error.message.includes("bcrypt")) {
      return responses.serverError("Password hashing failed", undefined, {
        requestId,
        event,
      });
    }
